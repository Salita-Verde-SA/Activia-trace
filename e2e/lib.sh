...
# themselves; -e would cause the runner to exit on first expected failure.
set -uo pipefail
...
# Only clears paths inside a sandbox HOME; never called on the real HOME.
cleanup_test_env() {
...
assert_exit_zero() {
    ...
    if "${cmd[@]}" >/dev/null 2>&1; then
        ...
        log_fail "Expected exit 0 for: ${cmd[*]}"
        return 1
    fi
}

# assert_exit_nonzero CMD... LABEL
# Runs a command and asserts exit code != 0.
assert_exit_nonzero() {
    local label="${!#}"  # last argument
    local cmd=("${@:1:$#-1}")  # all but last
    if ! "${cmd[@]}" >/dev/null 2>&1; then
        ...
        log_fail "Expected non-zero exit for: ${cmd[*]}"
        return 1
    fi
}
