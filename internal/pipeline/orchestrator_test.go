...
func TestOrchestratorPrepareFailureAbortsApply(t *testing.T) {
	order := []string{}
	orchestrator := NewOrchestrator(DefaultRollbackPolicy())

	result := orchestrator.Execute(StagePlan{
		Prepare: []Step{
			newRollbackStep("prep-1", &order, errors.New("prepare-boom")),
		},
		Apply: []Step{
			newTestStep("apply-1", &order),
		},
	...
	// Apply must not have run at all.
	if !reflect.DeepEqual(order, []string{"run:prep-1"}) {
		t.Fatalf("execution order = %v, want [run:prep-1]", order)
	}
}

func TestOrchestratorSkipsNonRollbackStepsDuringRollback(t *testing.T) {
	order := []string{}
	orchestrator := NewOrchestrator(DefaultRollbackPolicy())

	// plain step does NOT implement RollbackStep.
	result := orchestrator.Execute(StagePlan{
		Apply: []Step{
			newTestStep("plain-1", &order),              // no Rollback
			newRollbackStep("rb-2", &order, nil),        // has Rollback
			newRollbackStep("fail-3", &order, errors.New("fail")), // fails
		},
	})
	...
	// Rollback should call rb-2 but skip plain-1.
	if result.Rollback.Stage != StageRollback {
		t.Fatalf("rollback stage = %q", result.Rollback.Stage)
	}

	// plain-1 should not appear in rollback steps.
	for _, s := range result.Rollback.Steps {
		if s.StepID == "plain-1" {
			t.Fatalf("plain-1 should have been skipped during rollback")
		}
	}

	// rb-2 should appear as rolled back.
	found := false
	for _, s := range result.Rollback.Steps {
		if s.StepID == "rb-2" && s.Status == StepStatusRolledBack {
			found = true
		}
	}
	if !found {
		t.Fatalf("rb-2 should have been rolled back; steps = %v", result.Rollback.Steps)
	}
}
...
func TestRunnerNoProgressCallbackIsNoop(t *testing.T) {
	order := []string{}
	runner := Runner{FailurePolicy: StopOnError} // no OnProgress

	result := runner.Run(StageApply, []Step{newTestStep("s1", &order)})
...
// plainStep implements Step only — no Rollback.
...
func newTestStep(id string, order *[]string) *plainStep {
	return &plainStep{id: id, order: order}
}

func (s *plainStep) ID() string { return s.id }
func (s *plainStep) Run() error {
	*s.order = append(*s.order, "run:"+s.id)
	return s.runErr
}

// rollbackTestStep implements RollbackStep.
type rollbackTestStep struct {
	id      string
	order   *[]string
	runErr  error
	rollErr error
...
func newRollbackStep(id string, order *[]string, runErr error) *rollbackTestStep {
	return &rollbackTestStep{id: id, order: order, runErr: runErr}
}

func (s *rollbackTestStep) ID() string { return s.id }
func (s *rollbackTestStep) Run() error {
	*s.order = append(*s.order, "run:"+s.id)
	return s.runErr
}
func (s *rollbackTestStep) Rollback() error {
