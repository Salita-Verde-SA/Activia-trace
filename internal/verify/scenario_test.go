...
func TestScenarioReadyWhenAllChecksPass(t *testing.T) {
	...
	if report.FinalNote != readyMessage {
		t.Fatalf("FinalNote = %q, want %q", report.FinalNote, readyMessage)
	}
}

// ─── Spec: Hard check failure → not Ready ────────────────────────────────────
...
// ─── Spec: Soft check failure → warning, still Ready ─────────────────────────

func TestScenarioSoftCheckFailureIsWarning(t *testing.T) {
	checks := []Check{
		{
			ID:   "network-endpoint",
			Soft: true,
	...
	if !report.Ready {
		t.Fatalf("Ready = false, want true when only soft failures")
	}
	if report.Warnings != 1 {
		t.Fatalf("Warnings = %d, want 1", report.Warnings)
	}
	if report.Failed != 0 {
		t.Fatalf("Failed = %d, want 0", report.Failed)
	}
	...
	rendered := RenderReport(report)
	if !strings.Contains(rendered, "[??] network-endpoint") {
		t.Fatalf("RenderReport() missing warning line: %q", rendered)
	}
}

// ─── Mixed pass/fail/skip counts ─────────────────────────────────────────────
...
// ─── RenderReport formats all statuses ───────────────────────────────────────
