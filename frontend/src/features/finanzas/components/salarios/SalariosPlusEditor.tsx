  ...
  return (
    <div className="space-y-4 p-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-serif text-white/90">Salarios Plus (por Grupo)</h3>
      ...
      {(isCreating || editingId) && (
        <form onSubmit={handleSubmit} className="p-4 border border-white/10 rounded-xl bg-black/20 backdrop-blur-md space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div>
              <label className="block text-sm font-medium text-white/70">Nombre del Grupo</label>
              <input 
                type="text" 
                required
            ...
            <div>
              <label className="block text-sm font-medium text-white/70">Monto Plus ($)</label>
              <input 
                type="number" 
                required 
                min="0"
            ...
            <div>
              <label className="block text-sm font-medium text-white/70">Vigente Hasta</label>
              <input 
                type="date" 
      ...
      <div className="overflow-x-auto border border-white/10 rounded-xl shadow-sm bg-black/10 backdrop-blur-sm">
        <table className="min-w-full divide-y divide-white/10">
          <thead className="bg-white/5">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-white/50 uppercase tracking-wider">Grupo</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-white/50 uppercase tracking-wider">Rol</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-white/50 uppercase tracking-wider">Monto Plus</th>
          ...
          <tbody className="divide-y divide-white/10">
            {salariosPlusQuery.data?.map(salario => (
              <tr key={salario.id} className={`transition-colors ${salario.vigente_hasta ? "bg-black/40 opacity-75" : "hover:bg-white/5"}`}>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-white/90">{salario.grupo_nombre}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-white/70">{salario.rol}</td>
            ...
            {salariosPlusQuery.data?.length === 0 && (
              <tr>
                <td colSpan={6} className="px-6 py-4 text-center text-white/50">No hay salarios plus configurados.</td>
              </tr>
            )}
          </tbody>
        </table>
