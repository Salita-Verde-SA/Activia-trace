  ...
  return (
    <div className="space-y-4 p-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-serif text-white/90">Carreras</h3>
        ...
        <table className="min-w-full divide-y divide-white/10">
          ...
          <tbody className="divide-y divide-white/10">
            {carrerasQuery.data?.map(carrera => (
              <tr key={carrera.id} className="transition-colors hover:bg-white/5">
            ...
            {carrerasQuery.data?.length === 0 && (
              <tr>
                <td colSpan={4} className="px-6 py-4 text-center text-white/50">No hay carreras registradas.</td>
              </tr>
            )}
          </tbody>
        </table>
